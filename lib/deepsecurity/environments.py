...
class CloudAccounts(core.CoreDict):
    ...
    def get(self):
        ...
        call = self.manager._get_request_format(
            api=self.manager.API_TYPE_REST, call='cloudaccounts')
        ...
        if response and response['status'] == 200:
            if response['data'] and 'cloudAccountListing' in response['data'] and 'cloudAccounts' in response['data']['cloudAccountListing']:
                for cloud_account in response['data']['cloudAccountListing']['cloudAccounts']:
                    cloud_account_obj = CloudAccount(
                        self.manager, cloud_account, self.log)
                    self[
                        cloud_account_obj.cloud_account_id] = cloud_account_obj
    ...
    def add_aws_account(self, name, aws_access_key=None, aws_secret_key=None, region="all"):
        ...
        if region in regions:
            ...
            regions_to_add.append(list(regions.keys()))
        else:
            self.log(
                "A region must be specified when add an AWS account to Deep Security")
        ...
        for region_to_add in regions_to_add:
            ...
            if not responses[region_to_add] and responses[region_to_add]['status'] == 200:
                if responses[region_to_add]['raw'] and 'Cloud Account Region/Partition already present' in responses[region_to_add]['raw']:
                    self.log(
                        "The account/region you request has already been added to Deep Security. A specific account/region combination can only be added once")
