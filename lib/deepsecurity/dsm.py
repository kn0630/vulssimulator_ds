...
class Manager(core.CoreApi):
    ...
    def hostname(self, value):
        ...
        if value == 'app.deepsecurity.trendmicro.com':
            self.port = 443
    ...
    def _set_endpoints(self):
        ...
        """
        self._rest_api_endpoint = "https://{}:{}/rest".format(
            self.hostname, self.port)
        self._soap_api_endpoint = "https://{}:{}/webservice/Manager".format(
            self.hostname, self.port)
    ...
    def sign_in(self):
        ...
        if response and response['data']:
            self._sessions[self.API_TYPE_SOAP] = response['data']
        ...
        if response and response['raw']:
            self._sessions[self.API_TYPE_REST] = response['raw']
    ...
    def sign_out(self):
        ...
        if self._sessions[self.API_TYPE_SOAP]:
            ...
            if response and response['status'] == 200:
                self._sessions[self.API_TYPE_SOAP] = None

        # then the REST API
        rest_call = self._get_request_format(
            api=self.API_TYPE_REST, call='authentication/logout')
        if self._sessions[self.API_TYPE_REST]:
            response = self._request(rest_call)
            if response and response['status'] == 200:
                self._sessions[self.API_TYPE_REST] = None
    ...
    def get_api_version(self):
        ...
        # then the REST API
        rest_call = self._get_request_format(
            api=self.API_TYPE_REST, call='apiVersion')
        response = self._request(rest_call, auth_required=False)
        if response and response['status'] == 200 and response['data']:
    ...
    def get_time(self):
        ...
        if response and response['status'] == 200 and '#text' in response['data']:
            result = datetime.datetime.strptime(
                response['data']['#text'], "%Y-%m-%dT%H:%M:%S.%fZ")
    ...
    def is_up(self):
        ...
        result = None
        rest_call = self._get_request_format(
            api=self.API_TYPE_REST, call='status/manager/ping')
        response = self._request(rest_call, auth_required=False)
        if response and response['status'] == 200:
            result = True
        else:
    ...
    def assign_policy_to_computers(self, policy_id, computer_ids):
        ...
        soap_call = self._get_request_format(
            call='securityProfileAssignToHost')
        soap_call['data'] = {
            'hostIDs': computer_ids,
    ...
    def get_rule_recommendations_for_computer(self, computer_id):
        ...
        for rule_type, type_enum_val in list(rules_types.items()):
            rule_key = translation.Terms.get(rule_type).replace(
                '_retrieve_all', '').replace('_rule', '')
            results[rule_key] = []

            soap_call = self._get_request_format(
                call='hostRecommendationRuleIDsRetrieve')
            soap_call['data'] = {
                'hostID': computer_id,
                'type': type_enum_val,
