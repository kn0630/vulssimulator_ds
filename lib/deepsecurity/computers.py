    ...
    def get(self, detail_level='HIGH', computer_id=None, computer_group_id=None, policy_id=None, computer_name=None, external_id=None, external_group_id=None):
        ...
        if external_id and external_group_id:
            ...
            if computer_name.endswith('*'):
                call = self.manager._get_request_format(
                    call='hostDetailRetrieveByNameStartsWith')
                call['data'] = {
        ...
        if response and response['status'] == 200:
            ...
            for computer in response['data']:
                ...
                if computer_obj:
                    ...
                    self.log(
                        "Added Computer {}".format(computer_obj.id), level='debug')

                    try:
                        # add this computer to any appropriate groups on the
                        # Manager()
                        if 'computer_group_id' in dir(computer_obj) and computer_obj.computer_group_id:
                            if self.manager.computer_groups and computer_obj.computer_group_id in self.manager.computer_groups:
                                self.manager.computer_groups[computer_obj.computer_group_id].computers[
                                    computer_obj.id] = computer_obj
                                self.log("Added Computer {} to ComputerGroup {}".format(
                                    computer_obj.id, computer_obj.computer_group_id), level='debug')
                    except Exception as hostGroupid_err:
                        self.log("Could not add Computer {} to ComputerGroup".format(
                            computer_obj.id), err=hostGroupid_err)

                    try:
                        # add this computer to any appropriate policies on the
                        # Manager()
                        if 'policy_id' in dir(computer_obj) and computer_obj.policy_id:
                            if self.manager.policies and computer_obj.policy_id in self.manager.policies:
                                self.manager.policies[computer_obj.policy_id].computers[
                                    computer_obj.id] = computer_obj
                                self.log("Added Computer {} to Policy {}".format(
                                    computer_obj.id, computer_obj.policy_id), level='debug')
                    except Exception as securityProfileid_err:
                        self.log("Could not add Computer {} to Policy".format(
                            computer_obj.id), err=securityProfileid_err)
...
class ComputerGroups(core.CoreDict):
    ...
    def get(self, name=None, group_id=None):
        ...
        if name or group_id:
            ...
            if name:
                call = self.manager._get_request_format(
                    call='hostGroupRetrieveByName')
                call['data'] = {
                    'Name': name
                }
            elif group_id:
                call = self.manager._get_request_format(
                    call='hostGroupRetrieve')
                call['data'] = {
                    'id': '{}'.format(group_id)
                }
        else:
            call = self.manager._get_request_format(
                call='hostGroupRetrieveAll')
        ...
        if response and response['status'] == 200:
            ...
            for group in response['data']:
                computer_group_obj = ComputerGroup(
                    self.manager, group, self.log)
                if computer_group_obj:
                    self[computer_group_obj.id] = computer_group_obj
                    self.log("Added ComputerGroup {}".format(
                        computer_group_obj.id), level='debug')
...
class Computer(core.CoreObject):
    ...
    def get_recommended_rules(self):
        ...
        """
        self.recommended_rules = self.manager.get_rule_recommendations_for_computer(
            self.id)
...
class ComputerGroup(core.CoreObject):
    ...
    def send_events(self):
        ...
        """
        results = {}
        for computer_id, computer in list(self.computers.items()):
    ...
    def clear_alerts_and_warnings(self):
        ...
        """
        return self.manager.clear_alerts_and_warnings_from_computers(list(self.computers.keys()))

    def scan_for_malware(self):
        """
        Request a malware scan be run on all computers in this group
        """
        return self.manager.scan_computers_for_malware(list(self.computers.keys()))

    def scan_for_integrity(self):
        """
        Request an integrity scan be run on all computers in this group
        """
        return self.manager.scan_computers_for_integrity(list(self.computers.keys()))

    def scan_for_recommendations(self):
        """
        Request a recommendation scan be run on all computers in this group
        """
        return self.manager.scan_computers_for_recommendations(list(self.computers.keys()))

    def assign_policy(self, policy_id):
        """
        Assign the specified policy to all computers in this group
        """
        return self.manager.assign_policy_to_computers(policy_id, list(self.computers.keys()))
    ...
    def get_recommended_rules(self):
        ...
        for computer_id in list(self.computers.keys()):
            results[computer_id] = self.computers[
                computer_id].get_recommended_rules()
