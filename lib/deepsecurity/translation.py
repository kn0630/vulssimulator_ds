        ...
        if os.path.exists(terms_file_path):
            ...
            with open(terms_file_path, 'r') as fh:
                for line in fh:
                    ...
                    term_new = re.sub(
                        '([A-Z]+)', r'_\1', term).lower().lstrip('_')

                    # catch the term_new exceptions
                    if 'dpi' in term_new:
                        term_new = re.sub(
                            'dpi', '_intrusion_prevention_', term_new, re.IGNORECASE)
                    if 'host' in term_new:
                        term_new = re.sub(
                            'host', '_computer_', term_new, re.IGNORECASE)
                    if 'securityprofile' in term_new:
                        term_new = re.sub(
                            'securityprofile', '_policy_', term_new, re.IGNORECASE)
                    if 'integrity' in term_new:
                        term_new = re.sub(
                            '((?=integrity(?!_monitoring)))', '_integrity_monitoring_', term_new, re.IGNORECASE)
                        term_new = term_new.replace(
                            'integrity_monitoring_integrity', 'integrity_monitoring')
