...
# 3rd party libraries
from .libs import xmltodict as xmltodict
...
class CoreApi(object):
    ...
    def _request(self, request, auth_required=True):
        ...
        for required_key in [
            ...
            if required_key not in request and request[required_key]:
                self.log("All requests are required to have a key [{}] with a value".format(
                    required_key), level='critical')
                return None
        ...
        if 'query' in request and request['query']:
            ...
            for k, v in list(request['query'].items()):
                if v:
                    qs[k] = v

            url += '?%s' % urllib.parse.urlencode(qs)
            self.log(
                "Added query string. Full URL is now {}".format(url), level='debug')
        ...
        if self.ignore_ssl_validation:
            ...
            self.log(
                "SSL certificate validation has been disabled for this call", level='warning')
        ...
        if request['api'] == self.API_TYPE_SOAP:
            ...
            data = self._prep_data_for_soap(
                request['call'], request['data']).encode("utf-8")
            url_request = urllib.request.Request(
                url, data=data, headers=headers)
            request_type = 'POST'
            self.log(
                "Making a SOAP request with headers {}".format(headers), level='debug')
            self.log("   and data {}".format(data), level='debug')
        elif request['call'] == 'authentication/logout':
            url_request = urllib.request.Request(url, headers=headers)
            # make this request use the DELETE HTTP verb
            setattr(url_request, 'get_method', lambda: 'DELETE')
            request_type = 'DELETE'
            self.log(
                "Making a REST DELETE request with headers {}".format(headers), level='debug')
        elif 'data' in request and request['data']:
            # POST
            url_request = urllib.request.Request(
                url, data=json.dumps(request['data']).encode("utf-8"), headers=headers)
            request_type = 'POST'
            self.log(
                "Making a REST POST request with headers {}".format(headers), level='debug')
            self.log("    and data {}".format(request['data']), level='debug')
        else:
            # GET
            url_request = urllib.request.Request(url, headers=headers)
            self.log(
                "Making a REST GET request with headers {}".format(headers), level='debug')
        ...
        try:
            ...
            self.log("Failed to make {} {} call [{}]".format(
                request['api'].upper(), request_type, request['call'].lstrip('/')), err=url_err)
        ...
        self.log("Call returned HTTP status {} and {} bytes of data".format(
            result['status'], bytes_of_data), level='debug')
            ...
            if request['api'] == self.API_TYPE_SOAP:
                ...
                try:
                            ...
                            if '{}Response'.format(request['call']) in result['data']:
                                if '{}Return'.format(request['call']) in result['data']['{}Response'.format(request['call'])]:
                                    result['data'] = result['data'][
                                        '{}Response'.format(request['call'])]['{}Return'.format(request['call'])]
                                else:
                                    result['data'] = result['data'][
                    ...
                    self.log("Could not convert response from call {}".format(
                        request['call']), err=xmltodict_err)
            else:
                # JSON response
                try:
                    if result['raw']:
                    ...
                    self.log("Could not convert response from call {} to JSON. Threw exception:\n\t{}".format(
                        request['call'], json_err), level='info')
        return result
    ...
    def _prefix_keys(self, prefix, d):
        ...
        for k, v in list(d.items()):
            ...
            new_v = v
            if type(v) == type({}):
                new_v = self._prefix_keys(prefix, v)
            new_d[new_key] = new_v
            del(new_d[k])
    ...
    def _prep_data_for_soap(self, call, details):
        ...
        data = xmltodict.unparse(
            self._prefix_keys('ns1', {call: details}), pretty=False, full_document=False)
        soap_xml = """
...
class CoreDict(dict):
    ...
    def find(self, **kwargs):
        ...
        if kwargs:
            for item_id, item in list(self.items()):
                item_matches = False
                for match_attr, match_attr_vals in list(kwargs.items()):
                    if not type(match_attr_vals) == type([]):
                        match_attr_vals = [match_attr_vals]

                    # does the current item have the property
                    attr_to_check = None
                    if match_attr in dir(item):
                        attr_to_check = getattr(item, match_attr)
                    elif 'has_key' in dir(item) and match_attr in item:
                        attr_to_check = item[match_attr]

                    if attr_to_check:
                        # does the property match the specified values?
                        for match_attr_val in match_attr_vals:
                            if type(attr_to_check) in [type(''), type('')]:
                                # string comparison
                                match = re.search(
                                    r'{}'.format(match_attr_val), attr_to_check)
                                if match:
                                    item_matches = True
...
class CoreObject(object):
    ...
    def _set_properties(self, api_response, log_func):
        ...
        """
        for k, v in list(api_response.items()):
            val = v
            if 'has_key' in dir(v) and '@xsi:nil' in v and v['@xsi:nil'] == 'true':
                val = None

            new_key = translation.Terms.get(k)

            # make sure any integer IDs are stored as an int
            if new_key == 'id' and re.search('^\d+$', v.strip()):
                val = int(v)
            ...
            try:
                ...
                if log_func:
                    log_func(
                        "Could not set property {} to value {} for object {}".format(k, v, s))
                    try:
    ...
    def to_dict(self):
        ...
        api_properties = list(translation.Terms.api_to_new.values())
